...
interface WhatsAppButtonProps {
  ...
  message?: string
  buttonText?: string
  context?: string // For tracking click context
  className?: string
...
export default function WhatsAppButton({
  ...
  const processedMessage = Object.keys(templateParams).reduce((msg, key) => {
    return msg.replace(new RegExp(`{{${key}}}`, 'g'), templateParams[key])
  }, message)
  ...
  const handleClick = () => {
    // Track event before opening WhatsApp
  ...
  return (
    <a
      ...
      className={`inline-flex items-center px-4 py-2 border border-transparent text-base font-medium rounded-md text-white bg-green-600 hover:bg-green-700 transition-colors duration-300 ${className}`}
    >
